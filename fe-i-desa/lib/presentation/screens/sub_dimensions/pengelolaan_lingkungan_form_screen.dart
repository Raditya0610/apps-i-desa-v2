...
class _PengelolaanLingkunganFormScreenState extends ConsumerState<PengelolaanLingkunganFormScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_editingId != null) {
      result = await ref.read(pengelolaanLingkunganProvider.notifier).update(_editingId!, data);
    } else {
      result = await ref.read(pengelolaanLingkunganProvider.notifier).create(data);
    }
  ...
  void _resetFields() {
    _upayaMenjagaKelestarianLingkungan = null;
    _regulasiPelestarianLingkungan = null;
    _kegiatanPelestarianLingkungan = null;
    _pemanfaatanEnergiTerbarukan = null;
    _tempatPembuananganSampah = null;
    _pengelolaanSampah = null;
    _pemanfaatanSampah = null;
    _kejadianPencemaranLingkungan = null;
    _ketersediaanJamban = null;
    _keberfungsianJamban = null;
    _ketersediaanSepticTank = null;
    _pembuanganAirLimbahCairRumah = null;
  }

  void _prefillForm(PengelolaanLingkungan record) {
    setState(() {
      _editingId = record.id;
      _year = record.year;
      _upayaMenjagaKelestarianLingkungan = record.upayaMenjagaKelestarianLingkungan;
      _regulasiPelestarianLingkungan = record.regulasiPelestarianLingkungan;
      _kegiatanPelestarianLingkungan = record.kegiatanPelestarianLingkungan;
      _pemanfaatanEnergiTerbarukan = record.pemanfaatanEnergiTerbarukan;
      _tempatPembuananganSampah = record.tempatPembuananganSampah;
      _pengelolaanSampah = record.pengelolaanSampah;
      _pemanfaatanSampah = record.pemanfaatanSampah;
      _kejadianPencemaranLingkungan = record.kejadianPencemaranLingkungan;
      _ketersediaanJamban = record.ketersediaanJamban;
      _keberfungsianJamban = record.keberfungsianJamban;
      _ketersediaanSepticTank = record.ketersediaanSepticTank;
      _pembuanganAirLimbahCairRumah = record.pembuanganAirLimbahCairRumah;
    });
  }
  void _clearForm() {
    setState(() {
  ...
  Widget _buildRecordsList() {
    final provState = ref.watch(pengelolaanLingkunganProvider);
  ...
  Widget _buildRecordRow(PengelolaanLingkungan record) {
  ...
  Future<void> _confirmDelete(PengelolaanLingkungan record) async {
    ...
    final result = await ref.read(pengelolaanLingkunganProvider.notifier).delete(record.id!);
    if (mounted) {
