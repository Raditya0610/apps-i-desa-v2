...
class _AktivitasFormScreenState extends ConsumerState<AktivitasFormScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_editingId != null) {
      result = await ref.read(aktivitasProvider.notifier).update(_editingId!, data);
    } else {
      result = await ref.read(aktivitasProvider.notifier).create(data);
    }
  ...
  void _resetFields() {
    _kearifanBudayaSosial = null;
    _kearifanBudayaSosialDipertahankan = null;
    _kegiatanGotongRoyong = null;
    _frekuensiGotongRoyong = null;
    _keterlibatanWargaGotongRoyong = null;
    _frekuensiKegiatanOlahraga = null;
    _penyelesaianKonflikSecaraDamai = null;
    _peranAparatKeamananMediator = null;
    _peranAparatPemerintah = null;
    _peranTokohMasyarakat = null;
    _peranTokohAgama = null;
    _satuanKeamananLingkungan = null;
    _aktivitasSatuanKeamananLingkungan = null;
  }

  void _prefillForm(Aktivitas record) {
    setState(() {
      _editingId = record.id;
      _year = record.year;
      _kearifanBudayaSosial = record.kearibanBudayaSosial;
      _kearifanBudayaSosialDipertahankan = record.kearibanBudayaSosialDipertahankan;
      _kegiatanGotongRoyong = record.kegiatanGotongRoyong;
      _frekuensiGotongRoyong = record.frekuensiGotongRoyong;
      _keterlibatanWargaGotongRoyong = record.keterlibatanWargaGotongRoyong;
      _frekuensiKegiatanOlahraga = record.frekuensiKegiatanOlahraga;
      _penyelesaianKonflikSecaraDamai = record.penyelesaianKonflikSecaraDamai;
      _peranAparatKeamananMediator = record.peranAparatKeamananMediator;
      _peranAparatPemerintah = record.peranAparatPemerintah;
      _peranTokohMasyarakat = record.peranTokohMasyarakat;
      _peranTokohAgama = record.peranTokohAgama;
      _satuanKeamananLingkungan = record.satuanKeamananLingkungan;
      _aktivitasSatuanKeamananLingkungan = record.aktivitasSatuanKeamananLingkungan;
    });
  }
  void _clearForm() {
    setState(() {
  ...
  Widget _buildRecordsList() {
    final provState = ref.watch(aktivitasProvider);
  ...
  Widget _buildRecordRow(Aktivitas record) {
  ...
  Future<void> _confirmDelete(Aktivitas record) async {
    ...
    final result = await ref.read(aktivitasProvider.notifier).delete(record.id!);
    if (mounted) {
